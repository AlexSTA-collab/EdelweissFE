...
def computeJacobian(x: np.ndarray, element, quad_points, dim: int):
    ...
    if (dim - 1) == 1:
        ...
        gradients = element.tabulate(1, quad_points)[
            1:
        ]  # we only keep one component because the element has the same dependence in xi,eta
...
def compute_grad(x: np.ndarray, element, quad_points, nInt: int, dim: int):
    ...
    grad_phi_x_y_at_GPs_stacked = np.repeat(grad_phi_x_y_at_GPs[np.newaxis, :, :, :], dim, axis=0)
    # print("Determinant of the pseudojacobian:", sqrt_detG)
    return grad_phi_x_y_at_GPs_stacked, sqrt_detG


def compute_surface_grad(x: np.ndarray, element, quad_points, nInt: int, dim: int):
    """
...
def compute_surface_div_grad(x: np.ndarray, element, quad_points, nInt: int, dim: int):
    """
    ...
    # Shape: (node, component, dim, GP)
    if dim == 2:
        surface_div_phi_x_y_at_GPs_stacked = np.einsum("iqjab, ijq -> abq", grad_phi_x_y_at_GPs_stacked, T)
    elif dim == 3:
        surface_div_phi_x_y_at_GPs_stacked = np.einsum("iqjab, ijq -> abq", grad_phi_x_y_at_GPs_stacked, T)
    # Shape: (node, GP)
...
def interface_geometry(x: np.ndarray, element, quad_points, nInt: int, dim: int):
    ...
    for qp in range(grad_g.shape[0]):
        ...
        if dim == 2:
            ...
            tangents = np.einsum(
                "ijk,jk->ik", grad_g[qp], x[:, : int(x.shape[1] / 2)].transpose(1, 0)
            ...
            tangents = np.einsum(
                "ijk,jk->ik",
                grad_g[qp, :, :, :],
                x[:, : int(x.shape[1] / 2)].transpose(1, 0),
...
def calculate_B_surface_grad(grad, grad_s):
    ...
    print(f"PYTHON B_MATRIX SHAPE: {B_matrix.shape}")
...
def assign_K_grad_s_u_grad_s_v(B_matrix, Z_ijkl):
    ...
    K_grad_s_u_grad_s_v = 1.0 / 4.0 * K_grad_s_u_grad_s_v
