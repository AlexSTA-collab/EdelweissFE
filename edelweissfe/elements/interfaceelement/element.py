...
class InterfaceElement(BaseElement):
    ...
    def setMaterial(self, material: type):
        ...
        self.material = material  # It should contain C0_ijkl, CM_ijkl, CI_ijkl
    ...
    def computeYourself(
        ...
        # surface gradient increment ???

        for i in range(self._nInt):
            # get stress and strain
            force = self._stateVarsTemp[i][0:3]
            surface_stress = self._stateVarsTemp[i][3:12]
            ...
            # This should be done in time zero...

            # if time == 0:

            # C material tangent
            L0 = self._dStressdStrain[i][self._matrixVoigtIndices][
                :, self._matrixVoigtIndices
            ]
            LM = self._dStressdStrain[i][self._matrixVoigtIndices][
                :, self._matrixVoigtIndices : int(2 * self._matrixVoigtIndices)
            ]
            LI = self._dStressdStrain[i][self._matrixVoigtIndices][
                :, int(2 * self._matrixVoigtIndices) : int(3 * self._matrixVoigtIndices)
            ]

            # The interface finite elements work with the gradient of the displacement and not the strain
            # we follow a unified flux formulation as in Gu and He 2011 (This can be changed at a later stage without  a loss in generality)
            # i.e we symmetrize the material so that ε_{ij} C_{ijkl} ε_{kl} = u_{i,j} L_{ijkl} u_{k,l}
            # We achieve this by multiplying by two the shear moduli
            L0[3:, 3:] = L0[3:, 3:] * 2
            LM[3:, 3:] = LM[3:, 3:] * 2
            LI[3:, 3:] = LI[3:, 3:] * 2

            # Calculate the inverse of the material matrices (I need them once at the start step)
            Linv_0 = np.linalg.inv(L0)
            Linv_M = np.linalg.inv(LM)
            Linv_I = np.linalg.inv(LI)

            # Convert material Voigt matrix (matrixVoigtIndices,matrixVoigtIndices) to fourth order material tensor L_{ijkl}
            L0_ijkl = voigt_to_tensor(Linv_0)
            LM_ijkl = voigt_to_tensor(Linv_M)
            LI_ijkl = voigt_to_tensor(Linv_I)

            # Convert invrese material Voigt matrix (matrixVoigtIndices,matrixVoigtIndices) to fourth order material tensor S_{ijkl}
            S0_ijkl = voigt_to_tensor(Linv_0)
            SM_ijkl = voigt_to_tensor(Linv_M)
            SI_ijkl = voigt_to_tensor(Linv_I)
            ...
            Z_ijkl, H_inv_ij, H_inv_nF_ijk, Yn_H_inv_Fn_ijkl = (
                calculate_material_matrices(
                    self.n,
                    self.I,
                    self.J,
            ...
            detJ = lin.det(self.J[i])  # Jacobi determinant
            ...
            P_jumpv = assign_P_jumpv(self.grad, Nbasis, force, i)
            P -= P_jumpv * detJ * self._t * self._weight[i]

            # calculate P (surface elasticity contribution)
            P_grad_s_v = assign_P_grad_s_v(self.grad, grad_s, surface_stress, i)
